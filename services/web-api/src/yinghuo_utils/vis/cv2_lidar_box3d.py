...
def _second_det_to_nusc_box(detection):
    ...
    for i in range(box3d.shape[0]):
        quat = Quaternion(axis=[0, 0, 1], angle=box3d[i, 6]) # Rotate about z
        if box3d.shape[0] == 7:
            # waymo model
...
def visual(points, gt_anno, det, out_img_name, eval_range=35, conf_th=0.0):
