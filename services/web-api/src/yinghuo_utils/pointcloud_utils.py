...
class PC(object):
    ...
    @staticmethod
    def decode_rgb_from_pcl(rgb: np.array):
    ...
    @staticmethod
    def down_pcd_size(pcd: o3d.t.geometry.PointCloud, voxel_down_size=None, nb_neighbors=20, std_ratio=None) -> o3d.t.geometry.PointCloud:
        """
        voxel_down_size=0.02
        ...
        """
        if voxel_down_size is not None:
            pcd = pcd.voxel_down_sample(voxel_size=voxel_down_size)
    ...
    def calc_color(arr: list, range_min: float = .0, range_max: float = 1.0, color_map_name='rainbow'):
        ...
        rtn = PC.calc_color_rgb(
            arr, range_min=range_min, range_max=range_max, color_map_name=color_map_name).flatten()
        print('calc_color.', datetime.now())
        return rtn
