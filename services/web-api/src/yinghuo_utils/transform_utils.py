...
class Projector(object):
    ...
    def map_pointcloud_to_image(img, pc_points, extrinsic, intrinsic, min_dist=0.0, coloring=Coloring.DEPTH, intensity_vis_range=[20, 120]):
        ...
        if coloring == Projector.Coloring.DEPTH:
            ...
            intensities = np.clip(
                intensities, intensity_vis_range[0], intensity_vis_range[1])
...
class TransformUtils(object):
    ...
    @staticmethod
    def invert_arr4x4(matrix:np.ndarray):
        return pt.invert_transform(matrix)
    
    @staticmethod
    def invert_arr1x16(arr:list):
        return TransformUtils.invert_arr4x4(np.array(arr).reshape((4,4)))
    ...
    @staticmethod
    def psr_to_corners(p, s, r) -> np.array:
        """psr to 8 corner points
        ...
        Returns:
            np.array: [8,3]
        """
        trans_matrix = TransformUtils.euler_angle_to_rotate_matrix(r, p)
        ...
        world_coord = np.matmul(trans_matrix, np.transpose(local_coord)).T
    ...
    def proj_pts3d_to_img(pts, extrinsic_matrix, intrinsic_matrix):
        ...
        # if np.any(imgpos[2] < 0):
        #     return None

        imgpos2 = np.matmul(intrinsic_matrix, imgpos)
    ...
    def box_to_2d_points(box, extrinsic, intrinsic):
        ""
        box3d = TransformUtils.psr_to_corners(box[0], box[1], box[2])  # in lidar
        # imgpos = np.matmul(args.extrinsic_matrix, pts) # in camera

        box3d_corners_on_img = TransformUtils.proj_pts3d_to_img(
            box3d, extrinsic, intrinsic)
    ...
    @staticmethod
    def map_pointcloud_to_image(img_wh:list, 
            points_in_lidar:np.array, 
            extrinsic, intrinsic, min_dist=0.0, filter_by_dist=True):
        """_summary_

        Args:
            img_wh (list): [width, height]
            points_in_lidar (np.array): n,3
            extrinsic (_type_): _description_
            intrinsic (_type_): _description_
            min_dist (float, optional): _description_. Defaults to 0.0.

        Returns:
            _type_: points: n,2
        """
        assert points_in_lidar.shape[0] > 0
        assert points_in_lidar.shape[1] >=3
        
        img_width = img_wh[0]
        img_height = img_wh[1]
        
        pcd_arr = TransformUtils.cart2hom(points_in_lidar[:, :3])  # n,4
        pts_in_cam = (extrinsic @ pcd_arr.T).T  # n,4
        depth_to_cam = pts_in_cam[:, 2]

        points = TransformUtils.view_points(
            pts_in_cam.T[:3, :], intrinsic, normalize=True)  # 3, n

        mask = np.ones(depth_to_cam.shape[0], dtype=bool)
        if filter_by_dist:
            mask = np.logical_and(mask, depth_to_cam > min_dist)
            mask = np.logical_and(mask, points[0, :] > 1)
            mask = np.logical_and(mask, points[0, :] < img_width - 1)
            mask = np.logical_and(mask, points[1, :] > 1)
            mask = np.logical_and(mask, points[1, :] < img_height - 1)
            points = points[:, mask]

        return points.T[:, :2], mask, depth_to_cam
    ...
    def map_points_to_image(points: np.array,
        ...
        """
            points: (?, 3), fields: x,y,z
        """
        pcd_arr = TransformUtils.cart2hom(points[:, :3])  # n,4
        pts_in_cam = np.matmul(pcd_arr, extrinsic)
        points = TransformUtils.view_points(
            pts_in_cam.T[:3, :], intrinsic, normalize=True)
        return points.T
