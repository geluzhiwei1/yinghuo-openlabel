...
import { glObjectState } from '@/pointcloud/states'
    ...
    .then(async (res: any) => {
      ...
      for(const item of rtn) {
        if (jobConfig.frame === item.jobConfig.frame) {
          let datas = listify(item.frame_labels, (key, value) => value)
...
onMounted(() => {
  init()
  watch(() => glObjectState.layers.map.visible, async (newVal) => {
    if (newVal) {
      ...
      if (res && res.data && !_.isEmpty(res.data)) {
        globalStates.mainAnnoater.import('fileMap', res.data)
      }
    }
  })
})
