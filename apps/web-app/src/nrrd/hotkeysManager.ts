...
export class HotkeysManager {
  ...
  private listenHotkey(conf: any) {
    ...
    // 有 + 号，则表示组合键
    if (keys.includes('+')) {
      const ks = keys.toLocaleLowerCase().split('+').map((key) => key)
      this.watchers.push(
        whenever(() => (
          // 当前没有使用输入框，并且按了快捷键
    ...
    // 有 | 符号，则表示或者
      ...
      ks.forEach((key) => {
        ...
        this.watchers.push(
          whenever(() => notUsingInput.value && this.currentKeyboard.has(key), () => {
            cb?.()
          })
        )
      })
    ...
    // 只有单个按键
      ...
      this.watchers.push(
        // 当前没有使用输入框，并且按了快捷键
        whenever(() => notUsingInput.value
          && this.checkKeyCombination([keys.toLocaleLowerCase()]),
          () => cb?.())
      )
    }
  }
