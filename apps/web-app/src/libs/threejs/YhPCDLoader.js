...
class PCDLoader extends Loader {
	...
	parse( data ) {
		...
		// === WASM fast path ===
		// 优先调 rust_wasm 的 pc_parse_pcd,失败回退到下面原 JS 三分支循环。
		// 兜底逻辑:helper 不存在 / 抛错 / 返回 null 都走原路径。
		const helper = getRustHelper?.();
		if ( helper?.pc_parse_pcd ) {
			try {
				const formatCode = { ascii: 2, binary: 0, binary_compressed: 1 }[ PCDheader.data ];
				if ( formatCode !== undefined ) {
					const offsetsFlat = PCDheader.fields.map( ( f ) => PCDheader.offset[ f ] ).join( ',' );
					const out = helper.pc_parse_pcd(
						new Uint8Array( data ),
						PCDheader.headerLen,
						PCDheader.points,
						PCDheader.rowSize,
						PCDheader.fields.join( ',' ),
						PCDheader.size.join( ',' ),
						offsetsFlat,
						PCDheader.type.join( ',' ),
						formatCode,
					...
					if ( out ) {
						if ( out.position.length ) {
							position.length = 0;
							for ( let i = 0; i < out.position.length; i ++ ) position.push( out.position[ i ] );
						}
						if ( out.color ) {
							color.length = 0;
							for ( let i = 0; i < out.color.length; i ++ ) color.push( out.color[ i ] );
						}
						if ( out.normal ) {
							normal.length = 0;
							for ( let i = 0; i < out.normal.length; i ++ ) normal.push( out.normal[ i ] );
						}
						if ( out.intensity ) {
							intensity.length = 0;
							for ( let i = 0; i < out.intensity.length; i ++ ) intensity.push( out.intensity[ i ] );
						}
						if ( out.label ) {
							label.length = 0;
							for ( let i = 0; i < out.label.length; i ++ ) label.push( out.label[ i ] );
						}
						return { header: PCDheader, position, normal, color, intensity, label, rgb: color };
					}
				}
			} catch ( err ) {
				console.warn( '[wasm pcd] fallback to JS:', err );
			}
		}
		...
		// return {header:PCDheader, points: new Points( geometry, material )};
