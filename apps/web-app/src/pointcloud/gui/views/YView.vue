<template>
    <div id="y-view-manipulator" :style="[topDivStyle]" v-show="glGlobals.threeViews?.yView?.states?.visiable">
...
const handleClick = (mode: string) => {
  glGlobals.threeViews.yView.setContolMode(mode)
}

eventBus.on(eventBus.Common.WindowResized, () => {
  topDivStyle.value = {
    position: 'absolute',
    left: threeView.leftView.left + 'px',
    top: threeView.leftView.top + 'px',
    width: threeView.leftView.width + 'px',
    height: threeView.leftView.height + 'px',
  }
})
</script>
