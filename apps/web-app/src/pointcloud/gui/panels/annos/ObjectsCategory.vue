<template>
    <el-table-v2 v-loading="loading" :columns="tableColumns" :data="tableDataRef" :row-height="40" row-key="label_uuid"
...
import { MainAnnotator } from '@/pointcloud/tools/main-annotator'
import { mainAnnoStates } from '@/pointcloud/states'
...
const tableColumns: Column<any>[] = [
    ...
    },
    {
        ...
        cellRenderer: ({ rowData }) => {
            ...
            const onToggleVisible = () => {
                rowData.style.visible = !rowData.style.visible
                globalStates.mainAnnoater.updateVisible({
                    visible: rowData.style.visible,
                    objType: rowData.objType
                })
            }
            return (
                <div>
                    <el-button size="small" onClick={onSetting} circle aria-label={t('aria.focusObject')}>
                    ...
                    <el-button size="small" onClick={onToggleVisible} circle aria-label={t('aria.toggleVisibility')}>
                        <Icon icon={rowData.style.visible ? 'mdi:show-outline' : 'mdi:hide-outline'}></Icon>
                    </el-button>
    ...
    },
    {
        ...
        cellRenderer: ({ rowData }) => {
            return (
                <div style={{ width: '100%', backgroundColor: rowData.style.pointColor }}>
                    {rowData.objType}
                </div>
            )
        }
...
watch(() => mainAnnoStates.triger.objStylesUpdated,
    (newVal, oldVal) => {
        reloadTableData()
    })
