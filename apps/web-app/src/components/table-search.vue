	...
	<div class="search-container">
		<el-form ref="searchRef" :model="query" :inline="true">
			<el-form-item :label="item.label" :prop="item.prop" v-for="item in options" :key="item.prop">
				...
				<el-switch v-else-if="item.type === 'switch'" v-model="query[item.prop]" />
			</el-form-item>
			<el-form-item>
				<el-button  type="primary" @click="search"><Icon icon="lucide:search" />搜索</el-button>
				<el-button @click="resetForm(searchRef)"><Icon icon="lucide:refresh-cw" />重置</el-button>
			</el-form-item>
		</el-form>
	</div>
</template>
