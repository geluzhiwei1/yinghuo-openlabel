<template>
  <el-scrollbar height="600px">
    <el-tree ref="treeRef" v-model="valueRef" :data="treeData" show-checkbox node-key="name" default-expand-all
      :expand-on-click-node="false" :check-on-click-node="true" :check-strictly="true" @check="handleCheck">
      <template #default="{ node, data }">
        <span class="custom-tree-node">
          <span>{{ data.name }}</span>
          <span>
            <a v-if="node.isLeaf" :href="_.get(taxonomyState.taxonnomyInfo, 'ontologies[0]', '') + jobConfig.taxonomy + '#' + data.name"
              target="_blank">
              ...
            </a>
            <a style="margin-left: 8px" v-if="node.isLeaf" @click="handleClick(node, data)"> 选择 </a>
          </span>
        </span>
      </template>
    </el-tree>
    <el-row v-if="jobConfig.taxonomy.type === 'user'">
      <el-col :span="8">当前规范<el-button  type="success" size="small" circle @click="refresh()"><Icon icon="lucide:refresh-cw" /></el-button>： </el-col>
      <el-col :span="16">{{ jobConfig.taxonomy.name }}</el-col>
    </el-row>
    <el-row v-else>
      <el-col :span="8">当前规范<el-button  type="success" size="small" circle @click="refresh()"><Icon icon="lucide:refresh-cw" /></el-button>：</el-col>
      <el-col :span="16">{{ jobConfig.taxonomy.key + "（" + jobConfig.taxonomy.name + "）"}}</el-col>
    </el-row>
  </el-scrollbar>
</template>
...
import { ElScrollbar, ElText } from 'element-plus'
...
watch(() => taxonomyState.ontology, (newVal, oldVal) => {
