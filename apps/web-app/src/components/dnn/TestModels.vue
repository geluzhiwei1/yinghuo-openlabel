...
<script lang="tsx" setup>
  ...
  import { uiState, attrPanel, canvaPanel } from '@/states/UiState'
  ...
  const openFormEdit = (type: string, params) => {
    formRef.value.open(type, params)
  }
    ...
    },
    {
      ...
      cellRenderer: ({ rowData }) => (
        <>
          ...
          <ElLink href={ rowData.anno_href } target="_blank">标注</ElLink>
        </>
      ),
      width: 250,
      align: 'center',
  ...
  const loadData = () => {
    annoJobPerformApi.getList().then
    ((data) => {
