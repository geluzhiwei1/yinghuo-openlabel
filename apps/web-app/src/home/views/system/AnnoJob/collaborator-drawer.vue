<template>
    <el-drawer v-model="visible" :size="'50%'" destroy-on-close>
        ...
        <template #default>
            <el-text>
                将任务分配给部门或个人，由部门或个人完成任务
            </el-text>
            <h4>分配到部门</h4>
            <el-form :loading="loading">
                <el-form-item label="部门："><deptTreeSelect v-model:modelValue="formData.dept_id" /></el-form-item>
                <el-form-item>
                <el-button type="primary" @click="submitDeptForm()">
                    保存
                </el-button>
                </el-form-item>
            </el-form>
            ...
            <h4>分配到人</h4>
            <el-form :loading="loading">
                <el-form-item label="协作者："><teamMembersSelect v-model:modelValue="formData.collaborators" /></el-form-item>
                <el-button type="primary" @click="submitCollaForm()">
                    保存
                </el-button>
            </el-form>
        </template>
...
const submitDeptForm = () => {
    if (isEmpty(formData.dept_id)) {
        ...
        loading.value = true
        if (!batchMode.value) {
            annoJobPerformApi.update_collaborator(formData).then(() => {
            ...
            annoJobPerformApi.update_collaborators(dtos).then(() => {
