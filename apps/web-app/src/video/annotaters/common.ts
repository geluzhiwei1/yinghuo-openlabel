...
class BaseCanvas {
    ...
    constructor(containerEle: HTMLElement, canvasEle: HTMLElement) {
        this.containerEle = containerEle
        this.canvasEle = canvasEle

        this.canvasObj = new fabric.Canvas(canvasEle, {
            hoverCursor: 'default',
            selection: false,
            fireRightClick: true,
    ...
    public setBackgroundColor(color: string) {
        this.canvasObj.setBackgroundColor(color, () => {
    ...
    /**
     * Called whenever canvas zoom changes (currently from onMouseWheel).
     * Rescales stroke widths so vector strokes stay visually uniform across
     * zoom levels, and dispatches 'zoom:change' so subscribers (e.g., the
     * status bar zoom indicator) can react without polling.
     */
    public onZoomChange() {
        const w = 1
        ...
        this.dispatch("zoom:change", this.canvasObj.getZoom())
    }
...
export { BaseCanvas, Vector2, AnnoWorkEnum }