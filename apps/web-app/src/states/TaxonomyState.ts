...
const buildClassSchemaMap = (cla, map) => {
    // map[cla.name] = cla
...
async function getOntology(mission) {
    const params = { mission: mission }
    const data = await taxonomyApi.schema(params)
...
watch(()=>jobConfig.mission, async (newVal, oldVal) => {
...
const parseAnnoSpec = (data) => {
    ...
    taxonomyState.taxonnomyInfo.ontologies = _.get(data, 'openlabel.ontologies', [])
...
watch(()=>jobConfig.taxonomy, (newVal, oldVal) => {
  if (newVal === oldVal) {
      return
  }
  loadSpec()
